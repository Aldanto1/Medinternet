...
- **Поиск** — чат с ИИ. Приветствие показывается с круглой **иконкой робота слева**
  ...
  **Оценки уходят в RX Code:** id ответа приходит в финальном событии стрима
  (`message_id`), клиент шлёт его в `/api/ai/vote`, сервер вызывает
  `POST /api/chats/{chatId}/messages/{messageId}/like|dislike` (chatId берётся
  из `ai_sessions`). Снятие оценки в API не отправляется — эндпоинта «отменить» нет.
...
- **Оценки в истории не восстанавливаются** — при просмотре старого чата кнопки
  лайка/дизлайка не показывают ранее поставленную оценку (`messageId` ответов
  в своей БД не храним).
