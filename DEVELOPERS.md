...
- **chats** — история чатов Mini App: `id` (PK), `telegram_id`, `rx_chat_id` (uniq),
  ...
  сохранённом сообщении (RX Code AI переписку не отдаёт — дублируем у себя).
...
ссылка на сайт) + inline-навигация (Поделиться с другом / Как пользоваться) +
...
- **Поделиться с другом** — короткое приглашение + кнопки шеринга (Telegram/WhatsApp).
  Без реферальной программы (убрана по ТЗ от июля 2026).
...
нижней навигации нет — убраны по ТЗ от июля 2026). Страницы внутри приложения:
- **Поиск** — чат с ИИ. Под каждым ответом кнопки: копировать / лайк / дизлайк
  (оценки пока только на клиенте, отправка в API не реализована).
...
*Документ описывает состояние проекта на момент написания. Отправка лайков/дизлайков
в API, новая картинка главного сообщения, иконка робота у приветствия и кнопка
«Политика конфиденциальности» — отложены (ТЗ от июля 2026, ждут материалов/решений).*
